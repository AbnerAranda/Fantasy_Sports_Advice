...
for i in range(0, len(real_stat)):
    ...
    for j in range(1, games):
        player_df[p_stat][j] = round(((player_df[p_stat][j - 1] + player_df[r_stat][j - 1]) / (min_mean + player_df['MIN'][j - 1])) * min_mean)
    ...
    #The graph
    player_df.reset_index().plot(x="index", y=[r_stat, p_stat], linestyle='-', marker='o', color=['coral', 'limegreen'], figsize=(20, 7))  # ,'limegreen','silver','deepskyblue','coral'
    plt.title(pic_title)
    plt.xlabel("Game")
...
player_df_print = player_df.drop(columns=['SEASON_ID', 'Player_ID', 'Game_ID', 'GAME_DATE', 'MATCHUP', 'WL', 'VIDEO_AVAILABLE'])
print('\n')
...
for l in range(0, len(graph_label)):
    p_stat = pred_stat[l]
    r_stat = real_stat[l]
    print(graph_label[l] + " mean: " + str(player_df[r_stat].mean()))
    print(p_stat + " mean: " + str(player_df[p_stat].mean()) + ". The difference with the mean is: " + str(abs(player_df[r_stat].mean() - player_df[p_stat].mean())))
